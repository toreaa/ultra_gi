...
**When** user taps "START ØIKT" button
...
- Notification shows: "🏃 Økt-modus aktiv" + timer (updated every 10 seconds)
...
- **Session info card**: Planned session details (if planned) OR "Spontan økt" (if unplanned)
- **Status indicator**: "🟢 Aktiv" badge
- **Action buttons** (deferred to Story 5.3-5.5):
  - "Logg inntak" → Story 5.3
  - "Logg ubehag" → Story 5.4
  - "Avslutt økt" → Story 5.5

**MVP Scope for AC5:** Only timer display + session info. Action buttons can be disabled/placeholder.

## Out of Scope (Future Stories)
- ❌ Crash recovery dialog (Story 5.6 - Epic 5 stretch goal)
- ❌ "Pause session" functionality (future enhancement)
- ❌ Intake/discomfort logging (Story 5.3, 5.4)
- ❌ End session (Story 5.5)
- ❌ Scheduled notifications for fuel intake (Story 5.2)
- ❌ Dashboard "Start spontan økt" button (Epic 6)
- ❌ iOS support (defer to v1.1 - Android MVP first)
...
- [ ] Create foreground notification ("🏃 Økt-modus aktiv")
...
- [ ] Show "START ØIKT" button (before timer starts)
...
- [ ] Show "🟢 Aktiv" status badge
- [ ] Add placeholder buttons: "Logg inntak", "Logg ubehag", "Avslutt økt" (disabled)
...
### Testing
- [ ] Test navigation from FuelSelector → ActiveSession
...
### CRITICAL: Winston's POC Completed ✅

**POC Status:** Winston completed 4-hour background timer testing on 2025-10-24.
**Result:** ✅ SUCCESS - Hybrid Approach recommended (see `docs/architecture/EPIC5_BACKGROUND_TIMER_POC.md`)

**Key Findings:**
- ✅ 4-hour timer works with foreground service
- ✅ Battery drain: ~6% per hour (acceptable)
- ✅ Crash recovery: Auto-save every 10s to SQLite
- ⚠️ Manufacturer kills: May occur on Samsung/Xiaomi (needs testing)
- ✅ Notification updates: Every 10s (not 1s) for battery optimization
...
- ✅ AC1: Navigate from FuelSelector → ActiveSession with plannedSessionId
- ✅ AC2: SessionLogRepository.create() saves to database
- ✅ AC3: SessionTimer foreground notification + background auto-save
- ✅ AC4: Auto-save every 10 seconds (crash recovery)
- ✅ AC5: Timer UI (HH:MM:SS 72sp, session info, 🟢 Aktiv badge)

### Files Created/Modified
1. **src/services/SessionTimer.ts** (NEW - 185 lines)
   - SessionTimer class based on Winston's Hybrid Approach
   - Foreground notification (dismiss-proof)
   - UI updates every 1 second (smooth timer)
   - Notification + auto-save every 10 seconds (battery optimization)
   - Methods: start(sessionLogId, onTick), stop(), getCurrentElapsedSeconds()

2. **src/screens/session/ActiveSessionScreen.tsx** (NEW - 334 lines)
   - "START ØIKT" button → creates session log + starts timer
   - Large timer display (72sp, tabular-nums font)
   - Session info card (planned vs spontaneous)
   - 🟢 Aktiv status badge
   - Placeholder buttons (disabled): "Logg inntak", "Logg ubehag", "Avslutt økt"

3. **src/database/repositories/SessionLogRepository.ts** (MODIFIED +117 lines)
   - Added create(), getById(), updateDuration() methods
   - Bonus: getActiveSession(), endSession() for Story 5.5
   - Full TypeScript interfaces: SessionLog, CreateSessionLogData

4. **src/database/repositories/index.ts** (MODIFIED)
   - Export SessionLogRepository + types

5. **src/screens/session/FuelSelectorScreen.tsx** (MODIFIED)
   - Removed @ts-ignore for SessionActive navigation
   - Updated to use CompositeScreenProps (ProgramStack + RootStack)
   - Navigate to ActiveSession with plannedSessionId

6. **src/types/navigation.ts** (MODIFIED)
   - Added ActiveSession route: { plannedSessionId?: number }

7. **App.tsx** (MODIFIED)
   - Created RootStack navigator for modal screens
   - Added ActiveSession screen to RootStack

### Type Safety
- ✅ No new TypeScript errors
- ✅ All interfaces properly typed
- ✅ CompositeScreenProps allows FuelSelector to navigate to ActiveSession
- ✅ Removed @ts-ignore from FuelSelectorScreen

### Testing Notes
Manual testing recommended:
1. Navigate: ProgramDetail → SessionPlan → FuelSelector
2. Create fuel plan, tap "Bekreft plan"
3. Tap "Start økt" in Alert dialog → ActiveSessionScreen appears
4. Tap "START ØIKT" → Timer starts (00:00:00)
5. Verify: Foreground notification appears "🏃 Økt-modus aktiv"
6. Verify: Timer updates every 1 second on screen
7. Check database: session_logs table has new row with status='active'
8. Wait 10 seconds: Check database - duration_actual_minutes updated
9. Verify: Notification updates every 10 seconds

### Edge Cases Handled
- ✅ Spontaneous session: plannedSessionId = undefined works
- ✅ Database save error: Shows error message, stays on screen
- ✅ Timer cleanup: SessionTimer.stop() on unmount
- ✅ Placeholder buttons: Disabled with note "Loggefunksjoner kommer i Story 5.3-5.5"

### Known Limitations
- Action buttons (logg inntak/ubehag, avslutt økt) are placeholders (deferred to Story 5.3-5.5)
- Crash recovery dialog not implemented (Story 5.6)
- iOS not tested (Android MVP first)
- Battery drain testing pending (Winston's POC: ~6% per hour)
